...
def extrair_link_whatsapp_completo(dados):
    ...
    # CORREÇÃO AQUI: Removido (100%) e o encoding é forçado abaixo
    ...
    # CORREÇÃO AQUI: .encode('utf-8') impede interrogações
...
def gerar_proposta_html(dados):
    ...
    linhas = "".join([f"<tr><td><strong>{i['produto']}</strong><br><small style='color: #64748b;'>{i['especificacoes']}</small></td><td style='text-align:center;'>{i['quantidade']} un.</td><td style='text-align:right;'>R$ {i['valor_unitario']:.2f}</td><td style='text-align:right;'>R$ {(i['quantidade']*i['valor_unitario']):.2f}</td></tr>" for i in dados["itens"]])
    ...
    <body>
        <div class="container">
            <div class="header">{logo_tag} <div style="text-align:right; font-size:10px;"><b>{MARCA_FABRICANTE}</b><br>Emissão: {dados['data_geracao']}</div></div>
            ...
            <div style="text-align:right; font-size:16px; font-weight:bold;">TOTAL: R$ {(sum(i['quantidade']*i['valor_unitario'] for i in dados['itens']) - dados.get('desconto_valor', 0)):.2f}</div>
        </div>
    </body></html>
    """
