        ...
        <div class="container">
            ...
            <div class="conditions">
                ...
                <div class="bank-container">
                    <div><strong style="font-size: 11px; color: #0f172a;">📱 Escaneie ou acesse:</strong><br>👉 <a href="{LINK_PIX_OFICIAL}" target="_blank" style="color: #0284c7; font-weight: bold;">{LINK_PIX_OFICIAL}</a><br>💳 <strong>Titular:</strong> Ana Lúcia Zepelini &bull; <strong>Banco:</strong> Cora SCD (403)<br><strong>Agência:</strong> 0001 | <strong>Conta:</strong> 2515972-5</div>
                </div>
...
with aba1:
    if st.session_state.ultima_proposta:
        ...
        c_wsp.link_button("📱 Enviar no WhatsApp", p_info["link_wa"], type="primary", use_container_width=True)
        st.divider()

    fk = st.session_state.form_key
    st.subheader("1. Dados do Cliente")
    nome = st.text_input("Nome / Razão Social", placeholder="Ex: Ana Silva / Empresa X", key=f"cliente_{fk}")
    col_doc, col_wa = st.columns(2)
    doc = col_doc.text_input("CPF / CNPJ (para Cupom Fiscal/NF)", placeholder="Ex: 000.000.000-00", key=f"cpf_cnpj_{fk}")
    wa = col_wa.text_input("WhatsApp / Telefone", placeholder="Ex: (11) 99999-9999", key=f"wa_{fk}")
    ...
    st.divider()
    desconto = st.number_input("Desconto (R$)", min_value=0.0, value=0.0, key=f"desc_{fk}")
    col_pr, col_dt = st.columns(2)
    prazo = col_pr.text_input("Prazo (Dias)", value="10", key=f"prazo_{fk}")
    dt_entrega = col_dt.date_input("📅 Entrega", value=date.today(), format="DD/MM/YYYY", key=f"dt_{fk}")
    ...
    if st.button("🚀 GERAR, SALVAR E ZERAR FORMULÁRIO", type="primary", use_container_width=True):
        if not st.session_state.itens: st.error("Adicione itens!")
            ...
            # FIX: Leitura direta do estado usando as chaves dos inputs
            dados = {
                ...
                "cliente_cpf_cnpj": st.session_state.get(f"cpf_cnpj_{fk}", "N/A"),
