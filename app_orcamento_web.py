...
def gerar_proposta_html(dados):
    ...
    <head>
        ...
        <style>
            ...
            .container {{ max-width: 780px; margin: auto; background: #ffffff; padding: 20px; border-radius: 8px; border: 1px solid #e2e8f0; }}
    ...
    <body>
        <div class="container">
            ...
            <h3>Proposta Nº {dados['numero_proposta']}</h3>
            <div style="padding:10px; background:#f1f5f9; margin-bottom:20px;"><b>Cliente:</b> {dados['cliente_nome']} | <b>CPF/CNPJ:</b> {dados.get('cliente_cpf_cnpj', 'N/A')} | <b>WhatsApp:</b> {dados.get('cliente_wa', 'N/A')}</div>
...
with aba1:
    if st.session_state.ultima_proposta:
        ...
        st.success(f"✅ Proposta {p_info['numero']} ({p_info['cliente']}) gerada!")
    ...
    if st.button("🚀 GERAR PROPOSTA"):
        # AQUI O FIX: Lemos diretamente do state usando a chave, ignorando variáveis voláteis
        dados = {
