...
def extrair_link_whatsapp_completo(dados):
    ...
    # CORREÇÃO AQUI: .encode('utf-8') elimina interrogações
...
def gerar_proposta_html(dados):
    ...
    for item in dados["itens"]:
        ...
        linhas_tabela += f"""
        <tr>
            <td><strong>{item['produto']}</strong><br><small style="color: #64748b;">{item['especificacoes']}</small></td>
    ...
    # Adicionado meta charset e CSS de impressão A4
    return f"""
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        <p>Total: R$ {total_final:.2f}</p>
    </body>
    </html>
    """

# --- O RESTANTE DO SEU CÓDIGO (Interface e Histórico) ---
# ... (Cole aqui o restante do código que você já tinha, sem alterações) ...
