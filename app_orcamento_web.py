...
if entregas:
    st.error("⚠️ PROPOSTAS PARA ENTREGA HOJE!")
    for p in entregas:
        if st.button(f"➡️ {p['numero_proposta']} - {p['cliente_nome']}"):
            st.session_state.tab_active = "📋 Histórico & Pedidos"
...
with aba1:
    ...
    nome = st.text_input("Nome / Razão Social", value=e['cliente_nome'] if e else "", key=f"cliente_{fk}")
    c1, c2 = st.columns(2)
    doc = c1.text_input("CPF / CNPJ", value=e['cliente_cpf_cnpj'] if e else "", key=f"cpf_{fk}")
    wa = c2.text_input("WhatsApp", value=e['cliente_wa'] if e else "", key=f"wa_{fk}")
    ...
    with st.expander("🎨 Detalhes"):
        ...
        esp_t = c_esp1.text_input("Tema")
        esp_n = c_esp1.text_input("Nome")
        esp_c = c_esp1.text_input("Cor")
        esp_i = c_esp2.text_input("Idade")
        esp_g = c_esp2.text_input("Obs")
    q = st.number_input("Qtd", 1, key=f"q_{fk}")
    v = st.number_input("Valor", 10.0, key=f"v_{fk}")
    
    if st.button("➕ Adicionar"):
        det = f"Tema: {esp_t} | Nome: {esp_n} | Cor: {esp_c} | Idade: {esp_i} | Obs: {esp_g}"
        st.session_state.itens.append({"produto": prod, "especificacoes": det, "quantidade": q, "valor_unitario": v})
        st.rerun()
    
    if st.session_state.itens:
        for i, item in enumerate(st.session_state.itens): st.write(f"{item['produto']} - {item['quantidade']} un.")
        if st.button("🗑️ Limpar"): st.session_state.itens = []; st.rerun()

    st.divider()
    desc = st.number_input("Desconto (R$)", value=e['desconto_valor'] if e else 0.0, key=f"desc_{fk}")
    prazo = st.text_input("Prazo (Dias)", value=e['prazo_dias'] if e else "10", key=f"prazo_{fk}")
    frete = st.text_input("Frete", value=e['frete_tipo'] if e else "Retirada em Itatiba", key=f"frete_{fk}")
    
    if st.button("🚀 SALVAR PROPOSTA"):
        num = e['numero_proposta'] if e else f"PROP-{datetime.now().strftime('%Y%m%d%H%M')}"
        dados = {"numero_proposta": num, "data_geracao": datetime.now().strftime("%d/%m/%Y"), "data_entrega": date.today().strftime("%d/%m/%Y"), "cliente_nome": nome, "cliente_cpf_cnpj": doc, "cliente_wa": wa, "itens": list(st.session_state.itens), "desconto_valor": desc, "prazo_dias": prazo, "frete_tipo": frete, "pago": False, "entregue": False}
        ...
        st.session_state.itens = []; st.session_state.edit_mode = None; st.rerun()
